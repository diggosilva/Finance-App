...
//        FinanceSingleton.shared.save(finance ?? <#default value#>)
    }
    ...
    func handleStates(title: String?, description: String?, amount: String?, type: String?) {
        ...
        guard let title = title, !title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
              ...
              let amount = amount, !amount.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
              let type = type, !type.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            stateButton.value = .isOff
            return
        }
        
        addFinance(title: title, description: description, amount: amount, type: type)
